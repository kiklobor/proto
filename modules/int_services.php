...
// var_dump($urlArrf);
// echo '</pre>';
$catid = $categoryservices->getcatid($urlArrf);
// echo '<pre>';
// var_dump($catid);
// echo '</pre>';
// 	echo '<pre>';
...
// 	echo '</pre>';
  ...
  if ($cat != 'services') array_unshift($urlArrfl, $cat);
	$query = 'SELECT c.* FROM category AS c WHERE c.ID='.$catid." AND c.type='service'";
	// var_dump($query);
	...
	$from = 'services s';
	$where= "s.parent IN ('".$catss."') AND s.active=1";
	$groupby='s.ID';
...
$pquery = 'SELECT s.ID FROM '.$from.' WHERE '.$where.' GROUP BY '.$groupby.' ORDER BY s.name'; //СОБИРАЕМ ЗАПРОС
// var_dump($pquery);
...
<div class="contentContainer col-12 col-md-9 p-0">
	<div class="catalog w-100">
        ...
        foreach($cells as $cellval)
                //if(isset($cells[$i])) {
                if(TRUE) {
                    $buttonText= isset($_SESSION['cart'][$cellval['id']]) ? 'Заказано!' : 'Перейти к заказу';
