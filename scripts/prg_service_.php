...
$query="SELECT s.* FROM services s, category c WHERE s.parent=c.uID AND s.url=?s AND s.active=1 LIMIT 1";
...
if ($go->affectedRows()==1) {
 ...
 //   $pricesBlock=$cell;
  ...
  $ProductCountInCart = 1;
    ...
    // строка крошек
      $query = "SELECT c.name AS cname, c.ID AS cID, c.url AS curl, c.uID AS cuID FROM services s, category c WHERE s.ID=?i AND s.parent=c.uID AND c.type='service'";

      $parents=$go->getRow($query,$product['ID']);
      // var_dump($parents);

      if ($parents) {
        $cat = $categoryservices->getcatbyuid($parents['cuID']);
        if (count($cat)<>0) {
          $catpath = $cat['catpath'];
    			$link='';
    			while (count($catpath)<>0) {
    				$cat = $categoryservices->getlastcat($catpath);
    ...
    // блок описания
    $description= ($product['description']!='') ? '<span itemprop="description">'.$product['description'].'</span></br>' : '<meta itemprop="description" content="нет описания">';
    ...
    $similarSlider='';
    $query='SELECT ID FROM services WHERE active=1 AND ID NOT IN (?i) AND parent=?s';
    $similar=$go->getCol($query,$product['ID'],$product['parent']);
    if(count($similar)>0) {
        shuffle($similar);
        $similar=$go->getAll('SELECT * FROM services WHERE active=1 AND ID IN (?a)',array_slice($similar,0,8));
        $simImg=array();
        foreach ($similar as $val) {
            ...
            $block='
            <a href="/service/'.$val['url'].'" class="d-flex flex-column align-items-center p-1 simPr text-center m-1" style="min-height:300px;background-color:white;border:1px solid #bcbebd;overflow:hidden;">
