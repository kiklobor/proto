...
class SafeMySQL
{
	...
	protected $defaults = array(
		'host'      => 'localhost',
		'user'      => 'imigeru0_proto_test_userr',
		'pass'      => 'PvT^CkxB7.#q',
		'db'        => 'imigeru0_proto_test',
		'port'      => NULL,
		'socket'    => NULL,
		'pconnect'  => FALSE,
		'charset'   => 'utf8',
	...
	private $bot_token = '799513981:AAHdtCIqAtoVP8-P869C4tBSf4by8xirfEE';
    private $chat_id = '302711179';
    ...
    public function sendMessageAdmin($text){
        if ($text!='') {
            $adminsChats=$this->getAll("SELECT * FROM adminDelivery WHERE telegramID!=0 AND active=1");
            foreach($adminsChats as $key=>$val) {
                file_get_contents('https://api.telegram.org/bot'.$this->bot_token.'/sendMessage?chat_id='.$val['telegramID'].'&text='.urlencode($text));
                }
            return 0;
            }
        }
    ...
    public function sendMessage($text,$user){
        if ($text=='' || $user<=0) return false;
        $hour=date('H');
        if ($hour>8 && $hour<19) $url='https://api.telegram.org/bot'.$this->bot_token.'/sendMessage?chat_id='.$user.'&text='.urlencode($text);
        else $url='https://api.telegram.org/bot'.$this->bot_token.'/sendMessage?chat_id='.$user.'&disable_notification=true&text='.urlencode($text);
