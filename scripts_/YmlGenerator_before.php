...
abstract class YmlGenerator  {
    ...
    public $offerElements = array('url', 'price', 'currencyId', 'categoryId', 'market_category', 
            ...
            'rec','expiry','weight','dimensions','param', 'delivery-options', 'option', 'd_cost', 'days', 'order-before');
    ...
    protected function addOffer(
        ...
        $engine->startElement('delivery-options'); {
            $engine->startElement('option'); {
                ...
                $engine->writeAttribute('days', $availability);
                $engine->writeAttribute('order-before', 18); }
            $engine->endElement(); }
        $engine->endElement();
